...
class MyPredatorWithCommunication(MyPredator):
    ...
    def response(self, perceptions: MyAgentPerception) -> SocialAction:
        ...
        #Handle prey detected by other predators
        if not perceptions.nearby_prey:

            min_distance = 1000
            min_prey = None

            for incoming_msg in perceptions.messages:
                for (_, prey_pos) in incoming_msg.content['nearby_prey']:
            ...
            #There is info about indirect prey
            if min_prey != None:
            ...
            #There is no info about prey, even in the messages
            #Try to cover as much of the grid as possible, by distancing the predators from each other
            else:
                all_predator_positions = []
        
                # Add visible predators
                for (pred_id, pred_pos) in perceptions.nearby_predators:
                    if pred_id != self.id:  # Don't include self
                        all_predator_positions.append(pred_pos)
        
                # Add predator positions from messages
                    for incoming_msg in perceptions.messages:
                        if 'agent_position' in incoming_msg.content:
                            sender_pos = incoming_msg.content['agent_position']
                            all_predator_positions.append(sender_pos)
                
                if all_predator_positions:
        
                    for pred_pos in all_predator_positions:
                        # Only consider predators within a reasonable distance
                        distance = agent_pos.get_distance_to(pred_pos)
                        if distance <= 3:  # Adjust this threshold
                            rel_pos = agent_pos.get_simple_relative_orientation(pred_pos)
                
                            if rel_pos == GridRelativeOrientation.FRONT:
                                probability_map.remove(MyAction.NORTH)
                            elif rel_pos == GridRelativeOrientation.BACK:
                                probability_map.remove(MyAction.SOUTH)
                            elif rel_pos == GridRelativeOrientation.RIGHT:
                                probability_map.remove(MyAction.EAST)
                            elif rel_pos == GridRelativeOrientation.LEFT:
                                probability_map.remove(MyAction.WEST)
                            # Handle diagonal cases
                            elif rel_pos == GridRelativeOrientation.FRONT_LEFT:
                                # Predator is in front-left, so avoid both north and west
                                probability_map.remove(MyAction.NORTH)
                                probability_map.remove(MyAction.WEST)
                            elif rel_pos == GridRelativeOrientation.FRONT_RIGHT:
                                probability_map.remove(MyAction.NORTH)
                                probability_map.remove(MyAction.EAST)
                            elif rel_pos == GridRelativeOrientation.BACK_LEFT:
                                probability_map.remove(MyAction.SOUTH)
                                probability_map.remove(MyAction.WEST)
                            elif rel_pos == GridRelativeOrientation.BACK_RIGHT:
                                probability_map.remove(MyAction.SOUTH)
                                probability_map.remove(MyAction.EAST)
        ...
        #contents['nearby_predators'] = perceptions.nearby_predators
        contents['nearby_prey'] = perceptions.nearby_prey
        #contents['obstacles'] = perceptions.obstacles
        ...
        for predator_data in self._predator_agents:
            if not predator_data in agent_actions:
                ...
                if isinstance(predator_action, SocialAction):
                    ...
                    #if not predator_action.outgoing_messages:
                    for msg in predator_action.outgoing_messages:
                        self.message_box.append(msg)
                        # print("ADDED MESSAGE: ", msg.sender_id, msg.destination_id)

                    act = predator_action.action
                    #del predator_action
                    predator_action = act
...
if __name__ == "__main__":
    ...
    df.plot(kind="scatter", x="Step", y="Prey killed", xlabel="Step", ylabel="Prey killed", yticks=range(0, 11, 1), title="With Communication")
    plt.show()
